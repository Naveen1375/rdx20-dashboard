...
with st.sidebar:
    ...
    telemetry_file = st.file_uploader(
        ...
        type=["csv"],
        help="Upload your RDX20 signal analysis CSV export (signal_analys_*.csv)"
    )
    alarms_file = st.file_uploader(
        "Alarm Log CSV (.csv)",
        type=["csv"],
        help="Upload your alarm log CSV file"
    )

# ── Gate: show upload screen until both files are provided ──
...
def load_telemetry(file_bytes):
    # Wide-format signal analysis CSV from RDX20
    # Each signal has 16 sub-columns; we only need the "-Value" columns
    raw = pd.read_csv(file_bytes, encoding='utf-8-sig')

    # Map signal keyword → dashboard column name
    signal_map = {
    ...
    for keyword, col_name in signal_map.items():
        # Find the "-Value" column whose header contains this keyword
        match = [c for c in raw.columns if keyword in c and c.endswith("-Value")]
        if not match:
            raise ValueError(f"Could not find a '-Value' column for signal: '{keyword}'. "
                             f"Please check that your CSV is a valid RDX20 signal analysis export.")
        # Replace "null" strings with 0, coerce to numeric
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**📦 Ra Dataset**")
    st.caption(f"1,000 synthetic samples · Aluminium · Empirical power-law model")
